...
After a previous exploration in which we found that the two ileum samples for patient 5 are quite different, we try to clean them separately.
...
```{r}
root <- "../../Crohn_disease/"
out_dir <- paste0(root,"data/02_cleaned_data_emptyDrops/")
out_dir
```
...
```{r}
set.seed(100)
e.out <- emptyDrops(raw.total, lower = 25)
e.keep <- e.out$FDR <= 0.01
summary(e.keep)

```

From the paper 10.000 cells where loaded in the instruments for sequencing.

```{r}
c.keep <- defaultDrops(raw.total, expected=4000)
summary(c.keep)
```
...
```{r}
ECD.plot(obj,y_cut = 100)

```

So to start we drop all cell with library size lower then 100.
...
#obj <- drop.genes.cells(object = obj,cells = names(sizes[ sizes < 100]))
...
```{r}
c.keep <- defaultDrops(raw.total, expected=10000)
summary(c.keep)
```
We retain all cells that are detected by either EmptyDrops or CellRanger.
This makes it easier to compare the two methods later, rather than having to perform two separate analyses.
...
```{r}
dim(uninvolved.obj@raw)
```

```{r}
dim(involved.obj@raw)
```

There are some genes detected only in one sample.

```{r}
intersection.genes <- intersect(rownames(uninvolved.obj@raw),rownames(involved.obj@raw))

raw <- cbind(uninvolved.obj@raw[rownames(uninvolved.obj@raw) %in% intersection.genes,],involved.obj@raw[rownames(involved.obj@raw) %in% intersection.genes,])

dim(raw)
```

```{r}
exc.univolved <- uninvolved.obj@raw[!rownames(uninvolved.obj@raw) %in% intersection.genes,]
dim(exc.univolved)
```

```{r}
temp <- matrix(data = 0,nrow = dim(exc.univolved)[1],ncol = dim(involved.obj@raw)[2])
colnames(temp) <- colnames(involved.obj@raw)
exc.univolved <- cbind(exc.univolved,temp)
dim(exc.univolved)
```

```{r}
exc.ivolved <- involved.obj@raw[!rownames(involved.obj@raw) %in% intersection.genes,]
dim(exc.ivolved)
```

```{r}
temp <- matrix(data = 0,nrow = dim(exc.ivolved)[1],ncol = dim(uninvolved.obj@raw)[2])
colnames(temp) <- colnames(uninvolved.obj@raw)
exc.ivolved <- cbind(temp,exc.ivolved)
dim(exc.ivolved)
```

```{r}
identical(colnames(exc.ivolved),colnames(exc.univolved))
```

```{r}
temp <- rbind(exc.ivolved,exc.univolved)
raw <- rbind(raw,temp)
dim(raw)
```
...
```{r}
obj <- initRaw(obj,GEO="GSM3972010_GSM3972009",sc.method="10X",
...
```{r}
yset = 0.26#threshold to remove low UDE cells
plot.ude <- ggplot(nu_df, aes(x = n, y=nu)) + 
    geom_point(colour = "#8491B4B2", size=1) + 
     ylim(0,0.25) + xlim(0,1000) +
