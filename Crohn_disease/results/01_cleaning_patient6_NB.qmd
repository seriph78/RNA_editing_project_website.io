---
title: "Cleaning notebook patient 6"
editor: visual
execute: 
  freeze: true
---

Last compiled..... BRUTTO CAMPIONE
...
```{r}
patient <- "patient6_Uninvolved"
status <- "Uninvolved"
metadata <- readxl::read_excel("../data/tables-mmc2.xlsx",sheet = 1)
metadata <- as.data.frame(metadata[metadata$Patient.ID == "pat. 6" 
...
obj <- initializeMetaDataset(obj,
                             ...
                             sampleCondition = paste0("patient.6 ileum ",status))
```
...
```{r}
obj <- dropGenesCells(obj,cells = rownames(df$sizes[df$sizes$mit.percentage > 25,]))
df <- mitochondrialPercentagePlot(obj,splitPattern = "_",numCols = 1,genePrefix = "^MT-")
