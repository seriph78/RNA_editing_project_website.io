...
GDI.df[unlist(markers),]
```
```{r}
unlist(markers)[!unlist(markers) %in% rownames(GDI.df)]
```
```{r}
GDI.df[GDI.df$GDI > 1.5 &  rownames(GDI.df) %in% unlist(markers),]
```
```{r}
unlist(markers)[unlist(markers) %in% rownames(GDI.df[GDI.df$GDI > 1.5 &  rownames(GDI.df) %in% unlist(markers),])]
```
...
df <- read.csv("IBD_colitis_markers_clusters_expression_function_exp_markers.csv",row.names = 1)
...
dend =color_labels(dend,k=9)#,labels = rownames(pca_1),col=pca_1$colors)
dd <- dend %>%
...
tab <- grid.ftable(df, gp = gpar(fill = rep(c("grey80", "grey95"), each = (dim(df)[2]+1))),padding = unit(5, "mm"))
...
seurat.markers <- FindAllMarkers(seurat.data, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
```
```{r}
write.csv(seurat.markers,"seurat_markers.csv")
seurat.markers[unlist(markers),]
```

```{r}
seurat.markers$cluster <- paste("cl",seurat.markers$cluster,sep = ".") 
df.seu <- as.data.frame(matrix(nrow = length(names(markers)),ncol = length(unique(seurat.markers$cluster))))
rownames(df.seu) <- names(markers)
colnames(df.seu) <- unique(seurat.markers$cluster)
#not_ass_clusters <- NA
for (ro in colnames(df.seu)) {
  #print(ro)
  for (m in names(markers)) {
    pv <- seurat.markers[rownames(seurat.markers) %in% unlist(markers[[m]]) & seurat.markers$cluster == ro ,"p_val"]
    
    df.seu[m,ro] <- (sum(pv < 0.05)/length(unlist(markers[[m]])))*100
  }
}

df.seu <- round(df.seu,digits = 0)
df.seu
```

```{r,fig.width=18}
df.seu <- round(df.seu[,dend %>% labels], digits = 0)
colnames(df.seu) <- str_remove_all(colnames(df.seu), pattern = "cl.")
tab <- grid.ftable(df.seu, gp = gpar(fill = rep(c("grey80", "grey95"), each = (dim(df)[2]+1))),padding = unit(9.5, "mm"))
